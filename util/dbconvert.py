...
cur.execute('insert into person_info (uuid,name,sex,student_id,grade,department_no, sync) select uuid(),name,sex,student_id,grade,department_no,current_timestamp from bioverify.student_info')
cur.execute('commit')
...
cur.execute('insert into class (uuid,person_id,person_uuid,database_id,create_time,type,subtype) select bioverify.class.uuid,person_id,person_info.uuid,1017,bioverify.class.create_time,1,type from bioverify.class left join person_info on bioverify.class.student_id=person_info.student_id')
...
cur.execute('insert into sample (uuid,person_id,person_uuid,database_id,create_time,file,class_uuid,class_id) \
            ...
            1017,bioverify.sample.create_time,bioverify.sample.file,bioverify.sample.class_uuid,class.class_id from \
...
cur.execute('insert into template (uuid,person_id,person_uuid,database_id,modify_time,file,class_uuid,class_id,sample_uuid,sample_id, valid) \
            ...
            1017,bioverify.template.create_time,bioverify.template.file,bioverify.template.class_uuid,class.class_id, \
