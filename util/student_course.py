...
f = open(u'files/2010autumn/选课名单.csv')
...
for line in data[1:-1]:
    ...
    d.executeSQL(('update person_info set sex="%s",department_no="%s",name="%s",sync=current_timestamp where person_id = "%s"' % (gender,dept_id,s_name,person['person_id'])).encode('utf8'))
    ...
    if not tc:        
        d.executeSQL('insert into teacher_course (teacher_id, course_id, year, term, class_no) values ("%s","%s","%d","%d","%s")' % (t_id, course_id, 2010, 9, class_no))
        tc = d.getOneResult('select * from teacher_course where teacher_id = "%s" and course_id = "%s" and class_no = "%s"' % (t_id, course_id, class_no))
        #print tc['tc_id']
